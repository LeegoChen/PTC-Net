...
class TruncatedSmoothAP:
    ...
    def __call__(self, embeddings, positives_mask, negatives_mask):
        ...
        s_positives = s_qz.detach().clone()  # requires_grad_false
        ...
        s_sigmoid = sigmoid(s_diff, temp=self.tau1)  # (0,1) B,positives_per_query,B
        ...
        pos_mask = positives_mask.unsqueeze(1)  # B,1,B
        pos_s_sigmoid = s_sigmoid * pos_mask  # B,positives_per_query,B
        ...
        r = r_p / r_omega  # B, positives_per_query
        ...
        valid_positives_mask = torch.gather(positives_mask, 1, closest_positives_ndx)   # () tensor  B, positives_per_query
        masked_r = r * valid_positives_mask  # 正样本mask
        n_valid_positives = valid_positives_mask.sum(dim=1)  # n_valid_positives
